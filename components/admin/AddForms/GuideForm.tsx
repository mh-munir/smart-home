...
export default function GuideForm() {
  ...
  const [guideData, setGuideData] = useState<GuideData>({
    ...
    mainImagePreview: '',
    mainDescription: '',
    subSections: Array.from({ length: 2 }, () => ({
      subHeading: '',
      image: null,
      imagePreview: '',
      content: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'Failed to add guide');
      }
      ...
      console.error(error);
      alert(error instanceof Error ? error.message : 'Error creating guide');
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white shadow-2xl rounded-2xl border border-gray-100">
      <h2 className="text-3xl font-extrabold mb-8 text-gray-800 border-b pb-4">➕ Add New Guide</h2>
