...
const EditGuidePage = () => {
      ...
      try {
        ...
        if (mounted) {
          setGuideData({
            ...
            mainDescription,
            subSections: subSections.length > 0 ? subSections : Array.from({ length: 2 }, () => ({
              subHeading: '',
              image: null,
              imagePreview: '',
              content: '',
  ...
  useEffect(() => {
    previewsRef.current = [
      guideData.mainImagePreview,
      ...guideData.subSections.map((section) => section.imagePreview),
    ].filter(Boolean);
  }, [guideData.mainImagePreview, guideData.subSections]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (guideData.mainImage) {
        images.push(await fileToBase64(guideData.mainImage));
      }

      for (const section of guideData.subSections) {
        if (section.image) {
          images.push(await fileToBase64(section.image));
        }
      }

      let content = guideData.mainDescription;
      guideData.subSections.forEach((s) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'Failed to update guide');
      }
      ...
      console.error(error);
      alert(error instanceof Error ? error.message : 'Error updating guide');
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white shadow-2xl rounded-2xl border border-gray-100">
      <h2 className="text-3xl font-extrabold mb-8 text-gray-800 border-b pb-4">✏️ Edit Guide</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <section className="space-y-6">
          ...
          <div>
            <label htmlFor="title" className="block text-sm font-bold text-gray-700 mb-2">Guide Title</label>
            <input
              id="title"
              type="text"
              className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none transition"
              placeholder="e.g., How to Set Up Smart Home Devices"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setGuideData(prev => ({ ...prev, category: e.target.value }))}
              required
            />
          </div>

          <div>
            <label htmlFor="mainImage" className="block text-sm font-bold text-gray-700 mb-2">Main Guide Image</label>
            <input
              id="mainImage"
              type="file"
            ...
            {guideData.mainImagePreview && (
              <Image
                src={guideData.mainImagePreview}
                alt="Preview"
                unoptimized
                width={800}
                height={480}
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none transition"
              placeholder="Write the main guide description..."
              value={guideData.mainDescription}
              onChange={(e) => setGuideData(prev => ({ ...prev, mainDescription: e.target.value }))}
            ></textarea>
          </div>
        </section>

        {/* Steps 2-5+: Sub Sections */}
        {guideData.subSections.map((section, index) => (
