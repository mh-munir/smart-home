...
export default function EditGuidePage({ params }) {
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">✏️ Edit Guide</h1>
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-4">
        <div>
          <label className="block text-sm font-bold mb-1">Title</label>
          <input name="title" defaultValue={guide.title} className="w-full border p-2 rounded" required />
        </div>

        <div>
          <label className="block text-sm font-bold mb-1">Category</label>
          <input name="category" defaultValue={guide.category || ''} className="w-full border p-2 rounded" />
        </div>

        <div>
          <label className="block text-sm font-bold mb-1">Content (Markdown/HTML)</label>
          <textarea name="content" rows={10} defaultValue={guide.content || ''} className="w-full border p-2 rounded" />
        </div>
